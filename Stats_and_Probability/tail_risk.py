...
- MAD / standard deviation
  ...
  We can use the MAD/SD ratio as a gauge of fat-tailedness. The closer the number is to zero, the fatter the tails.
  The closer the number is to 1 (it can never exceed 1!), the thinner the tails.
...
class TailRisk:
    def __init__(self, data_to_download={'distr': 't_student', 'param': 2}, draws=5000):
        ...
        if self.data_to_download['distr'] == 't_student':
            ...
            self.prices = finv.download_time_series_and_put_in_df(self.data_to_download['param']).dropna()
            self.distribution = self.prices.pct_change()
...
# the ids / id should be a list; colors should have same length of ids
ids = [14331, 43828, 20753]

colors = ['darkslateblue', 'orangered', 'black']

tail_index_list = []
for i in range(1000):
    ts = TailRisk({'distr': 'gaussian', 'param': 2}, draws=1000).distribution
    ts = ts.sort_values(ascending=False)
    ts.index = range(len(ts.index))
    m = round(len(ts.index) * 0.1)
    tail_index = round(TailRisk.compute_hill_estimator(ts, m=m), 3)
    print(tail_index)
    tail_index_list.append(tail_index)

print('MEAN: ' + str(pd.Series(tail_index_list).mean()))
compute_stuff('gaussian', 1, colors=colors)
compute_stuff('ids', ids, colors=colors)
